...
# Tool paths
BBMAP="/raid/USERS/HOME/$(whoami)/bbmap/bbmap.sh"
FASTP="fastp"
UMI_TOOLS="umi_tools"
PEAR="pear"
SEQKIT="seqkit"
...
for fq1 in "$RAW_DIR"/*R1_001.fastq.gz; do
    ...
    $BBMAP ref="$REF_DIR/phix174_ill.ref.fa.gz" \
        in1="$fq1" in2="$fq2" \
        outu1="$PREPROC_DIR/${sample}_noPhiX_R1.fastq.gz" \
        outu2="$PREPROC_DIR/${sample}_noPhiX_R2.fastq.gz" \
    ...
    $BBMAP ref="$REF_DIR/rrna.fasta" \
        in1="$PREPROC_DIR/${sample}_dedup_R1.fastq.gz" \
        in2="$PREPROC_DIR/${sample}_dedup_R2.fastq.gz" \
        outu1="$PREPROC_DIR/${sample}_rrnaFilt_R1.fastq.gz" \
        outu2="$PREPROC_DIR/${sample}_rrnaFilt_R2.fastq.gz" \
    ...
    $PEAR -f "$PREPROC_DIR/${sample}_rrnaFilt_R1.fastq.gz" \
        -r "$PREPROC_DIR/${sample}_rrnaFilt_R2.fastq.gz" \
        -o "$PREPROC_DIR/${sample}_merged" \
        1>"$LOG_DIR/${sample}_pear.out" 2>"$LOG_DIR/${sample}_pear.err"

    # Adapter trimming, polyA/T trimming, quality filtering
    ...
    if [[ -f "$FASTP_INPUT" ]]; then
        ...
        echo "Warning: FASTP input $FASTP_INPUT not found, skipping trimming for $sample" | tee -a "$LOG_DIR/${sample}_fastp.err"
        continue
    fi
